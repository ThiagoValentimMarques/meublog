...
Ao assitir muitas reportagens e também lives (pricipalmente do [Átila Iamarino](https://www.youtube.com/channel/UCSTlOTcyUmzvhQi6F8lFi5w)) sobre o novo coronavírus, o processo de interiorização do vírus me chamou bastante atenção. Ao construir mapas com as informações do Ministério da Saúde (Figuras 6 e 7), nota-se que, de fato, o vírus está cada vez "invadindo" o interior do paí (para mais informações, consultar o [Portal Lagom Data](https://www.lagomdata.com.br/coronavirus) do Professor Marcelo Soares). Dos 5570 municípios brasileiros, 3701 (66,4%) já registraram pelo menos um caso e 1463 (26,3%) notificaram pelo menos um óbito por Covid-19. Esse processo é muito preocupante, visto que hospitais de cidades menos estruturadas não têm como tratar os pacientes com Covid-19 e tentarão transferência para centros maiores, que em muitos casos já estão colapsados. 
...
ggplot(geral2) + geom_sf(aes(fill = categ),color = NA)+
    scale_fill_manual(values = c("green","blue","pink","red"),
                    na.value = "white",
...
ggplot(geral2) + geom_sf(aes(fill = categ2),color = NA)+
    scale_fill_manual(values = c("white","green","blue","brown","yellow","pink","red"),
